...
describe CFoundry::V2::ModelMagic do
  ...
  describe "to_many relationships" do
    describe "reading" do
      context "with :as => :some_real_object" do
        let!(:mymodel) { fake_model { to_many :bars, :as => :foo } }
        let!(:othermodel) do
          fake_model :foo do
        ...
        describe "getting related objects" do
          it "uses the given object name in the request URI" do
            mock(client.base).get(
                "/v2/my_fake_models/#{myobject.guid}/foos", anything) do
              { :total_results => 0, :resources => [] }
            end

            myobject.bars
          end

          context "when it's inlined into the manifest" do
            it "use given object name for lookup" do
              myobject.instance_variable_set(:@manifest,
                { :metadata => { :guid => myobject.guid },
                  :entity => {
                    :foos => [
                      { :metadata => { :guid => otherobject.guid },
                        :entity => { :name => "a" }
                      }
                    ]
                  }
              ...
              expect(myobject.bars.size).to eq(1)
              expect(myobject.bars.first.name).to eq("a")
            end
          end
        end

        describe "getting the URL of the relationship" do
          it "reads from the manifest under the given object name" do
            stub(myobject).manifest do
              { :entity => { :foos_url => "http://example.com" } }
            end

            expect(myobject.bars_url).to eq("http://example.com")
          end
        end

        describe "bulk assignment" do
          let(:yetanotherobject) { othermodel.new(nil, client).fake }

          it "sets as the given object name in the manifest" do
            myobject.bars = [otherobject, yetanotherobject]

            expect(myobject.manifest[:entity][:foo_guids]).to eq([
              otherobject.guid, yetanotherobject.guid])
          end
        end
        ...
        describe "adding related objects" do
          it "uses the given object name in the request URI" do
            mock(client.base).put(
              "v2", :my_fake_models, myobject.guid, :foos, otherobject.guid, anything)

            myobject.add_bar(otherobject)
          end
        end

        describe "removing related objects" do
          it "uses the given object name in the request URI" do
            mock(client.base).delete(
              "v2", :my_fake_models, myobject.guid, :foos, otherobject.guid, anything)

            myobject.remove_bar(otherobject)
          end
        end
      end
    end
